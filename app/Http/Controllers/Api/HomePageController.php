...
class HomePageController extends Controller
{
    public function filter(Request $request){
		...
		$data = User::join('tbl_institute','users.id','=','tbl_institute.institute_id')->where(['users.role'=>'2',"users.id"=>$school_id])->select( '*')->get()->toArray();
        ...
        $filter=[$search_student,$search_session,$search_category,$getclass_id,$search_section,$school_id];
        if($data){
            $data=$this->studentDetails($data,$filter);
	...
	public function studentDetails($data,$filter)
    {
        
        if ($data || $filter) {
            $x = array_map(function ($index) use ($filter) {
                // print_r($filter[1]); exit;
                $newdata=User::join('tbl_student', 'users.id', '=', 'tbl_student.sid')
                ...
                ->where(['users.role'=>3,'tbl_student.school_id'=>$filter[5]])
                ->where('users.name','LIKE','%'.$filter[0].'%')
                ->where('tbl_student.session','LIKE','%'.$filter[1].'%')
                ->where('tbl_student.category_id','LIKE','%'.$filter[2].'%')
                ->where('tbl_student.class_id','LIKE','%'.$filter[3].'%')
                ->where('tbl_student.section_id','LIKE','%'.$filter[4].'%')
                ->get()->toArray();
