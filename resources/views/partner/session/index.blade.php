							...
							<table id="example1" class="table table-striped table-bordered display" style="width:100%">
								...
								<tbody>@foreach($users as $key => $value)
									<tr>
										<td>{{++$key}}</td>
										<td>{{$value->session}}</td>
										<td><a class="btn btn-outline btn-success mb-5" href="{{ url('partner/session/edit') }}/{{$value->id}}"><i class="fa fa-edit"></i></a>
                    						<a class="btn btn-outline btn-success mb-5" href="{{ url('partner/session/destroy') }}/{{$value->id}}" onclick="return confirm('Are you sure you want to delete this item?');"><i class="fa fa-trash-o"></i></a>
                    					</td>
									</tr>@endforeach
								</tbody>
					</table>
  ...
  <div class="modal center-modal fade" id="modal-center" tabindex="-1">
	  <div class="modal-dialog">
		<div class="modal-content">
		  ...
		  <form novalidate action="{{ url('partner/session/store') }}" method="POST">
