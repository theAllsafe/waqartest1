		  ...
		  <div class="row">
			...
			<div class="col-12">
			  <div class="box">
        		...
        		@endif
				<div class="box-body">
				    <form novalidate action="{{ url('partner/class/update/') }}/{{$class['id']}}" method="POST">
				        {{csrf_field()}}
                        {{method_field('PUT')}}
					@csrf
    					...
    					<div class="col-md-10">
    					    <div class="controls">
    					        <select class="form-control" name="category" required data-validation-required-message="This field is required">
        						    ...
        						    <option value="{{$row->id}}" @if($row->id==$class['category_id']) selected @endif>{{$row->category_name}}</option>
        						    @endforeach
    						    </select>
    					    </div>
    					</div>
    				...
    				<div class="form-group row">
    					...
    					<div class="col-md-10">
    					    <div class="controls">
    						<input class="form-control" type="text" value="{{$class['class']}}" name="class" required data-validation-required-message="This field is required">
    					    </div>
    					</div>
    				</div>
